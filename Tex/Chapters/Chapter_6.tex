...
\item Sei $f: \lbrack a,b\rbrack \to \lbrack 0,\infty)$ eine (stetige) Funktion. Gesucht ist eine vernünftige Definition des Flächeninhalts A des Gebietes zwischen der $x$-Achse und dem Graphen von $f$
\begin{center}
\begin{tikzpicture}
...
%\def \myVar {10}
%\definecolor{color1}{RGB}{240,19,37}
%\definecolor{color2}{RGB}{31,29,28}
%\definecolor{color3}{RGB}{62,43,255}
%
%
\def \myVar {40}
\definecolor{color1}{RGB}{175,175,175}
\definecolor{color2}{RGB}{100,100,100}
\definecolor{color3}{RGB}{10,10,10}




\filldraw[draw=color1, fill=color1!\myVar] (0,0) rectangle ++(2,2.65);
\filldraw[draw=color1, fill=color1!\myVar] (2,0) rectangle ++(3,2.71);
\filldraw[draw=color1, fill=color1!\myVar] (5,0) rectangle ++(1.5,2.35);
\filldraw[draw=color1, fill=color1!\myVar] (6.5,0) rectangle ++(3,5);

\filldraw[draw=color2, fill=color2!\myVar] (0,0) rectangle ++(2,2);
\filldraw[draw=color2, fill=color2!\myVar] (2,0) rectangle ++(3,2.5);
\filldraw[draw=color2, fill=color2!\myVar] (5,0) rectangle ++(1.5,2.2);
\filldraw[draw=color2, fill=color2!\myVar] (6.5,0) rectangle ++(3,4);


\filldraw[draw=color3, fill=color3!\myVar] (0,0) rectangle ++(2,1.38);
\filldraw[draw=color3, fill=color3!\myVar] (2,0) rectangle ++(3,2.12);
\filldraw[draw=color3, fill=color3!\myVar] (5,0) rectangle ++(1.5,2.09);
\filldraw[draw=color3, fill=color3!\myVar] (6.5,0) rectangle ++(3,2.35);
...
\draw	(9.5,4.5) node[anchor=west] {$\color{color1}O(f,P)$}
		(9.5,3.25) node[anchor=west] {$\color{color2}S(f,P,\xi)$}
		(9.5,1.25) node[anchor=west] {$\color{color3}U(f,P)$};
...
\pgfmathdeclarefunction{poly}{0}{%
      \pgfmathparse{-x^3+5.25*(x^2)-3*x-3}%
    }

\begin{tikzpicture}[scale=1.1]
\filldraw[draw=black!15, fill=black!15] (0.79,0) rectangle ++(5.58,3.035);
\begin{axis}[
  xmin=-2,
  xmax=5,
  ymin=-5,
...
\addplot[mark=none,help lines,domain=-2:4.5] {3};

% draw graph for the first function f
\addplot+[black,thick,mark=none,domain=-1.2:4.5,stack plots=y] {poly};
% draw graph of max(g-f, 0) and stack
\addplot+[mark=none,fill=gray!70,draw=black,domain=-1.2:4.5,stack plots=y] {max(3-(poly),0)} \closedcycle;
% draw graph of min(g-f, 0) and stack
\addplot+[mark=none,fill=black!60,draw=black,domain=-1.2:4.5,stack plots=y] {min(3-(poly),0)} \closedcycle;

\draw[help lines] (axis cs:-1.2,-5) -- (axis cs:-1.2,3);
\draw[help lines] (axis cs:1.84,-5) -- (axis cs:1.84,3);
\draw[help lines] (axis cs:4.5,-5) -- (axis cs:4.5,3);
\end{axis}

\draw	(0.95,3.6) node[anchor=north] {D}
		(2.3,2.5) node[anchor=north] {A}
		(4.9,3.8) node[anchor=north] {C}
		(6.23,3.1) node[anchor=north] {\footnotesize B}
		;
\end{tikzpicture}
\end{center}
...
\hline
    Definitionsbereich & Funktion $f$                          & Stammfunktion $F$                                                      \\\hline\hline
...
Sei $f:\lbrack 1,\infty )\to \R_+$ monoton fallend. Dann konvergiert $\sum\limits_{k = 1}^\infty  {f(k)}$ genau dann wenn $\int\limits_1^\infty  {f(x)dx} $ existiert. In diesem Fall gilt: \[0 \le \sum\limits_{k = 1}^\infty  {f(k)}  - \int\limits_1^\infty  {f(x)dx}  \le f(1)\]
\subsubsection*{Beweis}
\begin{center}
\begin{tikzpicture}[scale=0.7]
% horizontal axis
...
Aus \[\sum\limits_{k = 1}^{n - 1} {f(k) \ge \int\limits_1^n {f(x)dx} } \] folgt, dass \[\sum\limits_{k = 1}^\infty  {f(k) < \infty  \Rightarrow \int\limits_1^\infty  {f(x)dx < \infty } } \] und aus \[\int\limits_1^n {f(x)dx \ge \sum\limits_{k = 1}^{n - 1} {f(k + 1)} } \] folgt, dass \[\int\limits_1^\infty  {f(x)dx < \infty  \Rightarrow \sum\limits_{k = 1}^\infty  {f(k) < \infty } } \]
