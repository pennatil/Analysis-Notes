...
Sei $f:\lbrack 1,\infty )\to \R_+$ monoton fallend. Dann konvergiert $\sum\limits_{k = 1}^\infty  {f(k)}$ genau dann wann $\int\limits_1^\infty  {f(x)dx} $ existiert. In diesem Fall gilt: \[0 \le \sum\limits_{k = 1}^\infty  {f(k)}  - \int\limits_1^\infty  {f(x)dx}  \le f(1)\]
\subsubsection*{Beweis}
\begin{center}
\begin{tikzpicture}[scale=0.7]
% horizontal axis
