...
\end{align*}
Wegen der Stetigkeit der Funktionen \[\frac{{\partial f}}{{\partial x}}\left( {x,y} \right) = {e^y}\hspace{5mm}\text{und}\hspace{5mm}\frac{{\partial f}}{{\partial y}}\left( {x,y} \right) = x{e^y}\] können wir den ``Fehler'' $R\left( x,y\right)$ leicht abschätzen \[\frac{{\abs{ {R\left( {x,y} \right)} }}}{{\abs{ {\left( {x,y} \right) - \left( {{x_0},{y_0}} \right)} }}} \le \mathop {\sup }\limits_{\begin{array}{*{20}{c}}
{\abs{ {\xi  - {x_0}} } < \abs{ {x - {x_0}} }}\\
{\abs{ {\eta  - {y_0}} } < \abs{ {y - {y_0}} }}
\end{array}} \left( {\abs{ {{e^y} - {e^{{y_0}}}} } + \abs{ {{x_0}} }\abs{ {{e^\eta } - {e^{{y_0}}}} }} \right)\]
...
Sei $f:\Omega\to\R$ differenzierbar an der Stelle $x_0\in\Omega$ und sei $e\in\R^n\backslash\{ 0\}$; mit $\abs{ e}=1$. Betrachte die Gerade $g(t)=x_0+te$, $t\in\R$ durch $x_0$ mit Richtungsvektor
