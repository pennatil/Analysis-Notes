...
Eine Menge $K\subset \R^n$ ist genau dann konvex, falls für jedes Paar von Punkten $x,y\in K$ die Menge $K$ auch das Segment \[\left( 1-t\right) x+ty\hspace{10mm} t\in\lbrack 0,1\rbrack\] mit Endpunkten $x,y$ enthält
\begin{multicols}{2}
\begin{center}
\begin{tikzpicture}[scale=0.75]
\node [] (0) at (1, 1) {};
...
\begin{enumerate}
    \item \begin{align*}
 f:\R^3 &\to\R\\
 \left( x,y,z\right) &\mapsto x^2 +y^2 +z^2
 \end{align*}
dann ist $f^{-1}(s)=$ Sphäre mit Zentrum $O$ und Radius $\sqrt{s}$
\item $f\left( x,y\right) = xy$ ist ein hyperbolischer Parabolid mit Niveaulinien \missingfigure{page 158, bottom, use minipage\\\textcolor{red}{ARE YOU SURE THIS IS NOT $x^2-y^2$? I CAN'T REPLICATE YOUR DRAWING HERE}} \\

$f:\R^n \to\R$. Nun sei $x_0\in\Omega$ mit $f\left( x_0\right) = s$, i.e. $x_0 \in f^{-1} (s)$. Sei $\gamma :\lbrack -1,1\rbrack\to \R^n$ ein differenzierbare Kurve durch $x_0$ mit $\gamma\lbrack -1,1\rbrack\subset f^{-1}(s)$, $\gamma (0)=x_0$
...
\draw (-2.5,-1.5) node[anchor=south] {$\nabla f\left( x_0\right)$};
...
\begin{align*}
v:\Omega &\to\R^n\\
x &\mapsto\left( v^i(x)\right)_{i=1}^n
\end{align*}
ein stetiges Vektorfeld ($v^i(x):\R^n\to\R$ stetig) dann ist durch $\lambda (x)(\omega)\defeq {< v(x),\omega >}$ eine $1-$Form $\lambda (x)\in L\left( \R^n,\R\right)$ definiert
\begin{align*}
...
t &\mapsto\left( \gamma^1 (t), \gamma^2 (t),\dots ,\gamma^n (t) \right)
\end{align*}
ein $C^1-$Weg. Dann ist
\begin{align*}
\lbrack a,b\rbrack &\to \R\\
t&\mapsto \lambda\left( \gamma (t)\right)\left( \gamma' (t)\right) = \sum{\lambda_i\left(\gamma(t)\right)\cdot\frac{d\gamma^i}{dt}(t)}
\end{align*}
eine stetige Funktion. Somit ist das Integral $\int\limits_a^b {\lambda \left( {\gamma \left( t \right)} \right)\left( {\gamma '\left( t \right)} \right)dt} $ wohldefiniert.

\begin{definition}{8.36}
Das Wegintegral von $\lambda\in L\left( \R^n,\R\right)$ längs $\gamma$ ist \[\int\limits_\gamma  \lambda  : = \int\limits_a^b {\lambda \left( {\gamma (t)} \right)\left( {\gamma '(t)} \right)dt} \]
\end{definition}
...
eine Parametrisierung des Einheitskreises und $\lambda = \lambda (x,y)$ die $1-$Form mit \[\lambda (x,y)=-ydx+xdy\hspace{10mm}(x,y)\in\R^2\]
Dann gilt
\begin{align*}
\int\limits_\gamma  \lambda   = &\int\limits_0^{2\pi } {\underbrace {\left( { - \sin t,\cos t} \right)}_{\lambda (\gamma (t))} \cdot \underbrace {\begin{pmatrix} -\sin t \\ \cos t \end{pmatrix}}_{\gamma '(t)}dt} \\
...
\int\limits_\gamma  {df}  &= \int\limits_a^b {df\left( {\gamma \left( t \right)} \right)\gamma '\left( t \right)dt} \\
 &=  \int\limits_a^b {\frac{d}{{dt}}\left( {f \circ \gamma } \right)\left( t \right)dt}\\
...
Sei $\lambda = 2xy^2dx+2x^2ydy$. \\

\noindent\underline{Ansatz:} \[f\left(x,y\right)=\int\limits_{\gamma_{\left( x,y\right)}}\lambda\]
wobei $\gamma_{\left( x,y\right)}\left(t\right)=\left(tx,ty\right)$, $t\in\left( 0,1\right)$. Dann ist
...
Im nächsten Kapitel, mittels höheren partiellen Ableitungen, erhalten wir eine einfache zu handhabende notwendige Bedingung für ein konservatives Vektorfeld. Wir werden sehen, dass
...
Für ein beliebiges $m$, heisst die Funktion $f\in C^1\left( \omega\right)$ von der Klasse $C^m$, $f\in C^m\left( \omega\right)$, falls $\frac{\partial f}{\partial x^i}\in C^{m-1}\left( \Omega\right)$, $1\leq i\leq n$\\
...
\begin{enumerate}
\item \[v\left( {x,y} \right) = \begin{pmatrix} 4x{y^2} \\ {2y} \end{pmatrix} \]
Es gilt $\frac{\partial v^1}{\partial y}=8xy$, $\frac{\partial v^2}{\partial x}=0$. Also ist $v$ nicht konservativ.
\item Sei $\Omega = \left\{ \left( x,y\right) \in\R^2:\left( x,y\right) \not= (0,0) \right\} = \R^2\setminus\left\{ 0,0\right\}$ und \[v(x,y) = \left( {\frac{{ - y}}{{{x^2} + {y^2}}},\frac{x}{{{x^2} + {y^2}}}} \right)\] Dann $v:\Omega \to\R^2$ mindestens $C^1$. Ausserdem \[\frac{{\partial v^1}}{{\partial y}} = \frac{{{y^2} - {x^2}}}{{{{\left( {{x^2} + {y^2}} \right)}^2}}} = \frac{{\partial {v^2}}}{{\partial x}}\] Jetzt berechnen wir $\int\limits_C v ds$, wobei C:
...
&= \int\limits_0^{2\pi } \left( -\sin t,\cos t\right)\cdot  \begin{pmatrix} -\sin t \\ \cos t \end{pmatrix} dt\\
&= \int\limits_0^{2\pi } \left( \sin^2 t+\cos^2 t\right) dt=2\pi\not= 0\\
...
$$\Rightarrow\text{ $v$ ist konservativ auf $\Omega$}$$
Das heisst die Konservativität ist eine Eigenschaft zugleich des Vektorfeldes $v$ \underline{v} und der Region $\Omega$.
\end{enumerate}

\begin{definition}{}
Sei $v$ ein Vektorfeld $v:D\to\R^2$. Die skalare \emph{Rotation} ist definiert als \[ \rot{v(x,y)}\defeq \frac{\partial v_2}{\partial x}(x,y)-\frac{\partial v_1}{\partial y}(x,y) \]
\end{definition}

$\rot{v(x,y)} = 0$ ist eine notwendige Bedingung für die Existenz eines Potentials. Die Bedingung $\rot{v(x)=0}$ ist hinreichend, falls das Gebiet $D$ einfach zusammenhängend ist, also keine ``Löcher'' enthält. 

\begin{definition}{8.49}
Eine offene Menge $\Omega\subset\R^n$ heisst einfach zusammenhängend, falls
\begin{enumerate}
\item $\Omega$ stückweise $C^1-$wegzusammenhängend ist.
\item jeder stückweise $C^1-$Weg in $\Omega$ stetig innerhalb $\Omega$ auf einen Punkt zusammengezogen werden kann.
\end{enumerate}
Die Region $\Omega=\R\setminus\{ 0\}$ ist nicht einfach zusammenhängend. \\ $\Omega' = \left\{ \left( x,y\right) \mid x>0\right\}$ ist es aber.
\end{definition}
...
\begin{enumerate}
\item $v$ ist konservativ
\item $\frac{\partial v^1}{\partial y}=\frac{\partial v^2}{\partial x}$
\end{enumerate}

\subsection*{Taylorentwicklung und das lokale Verhalten von $C^m-$Funktionen}\todo[inline]{Not sure how big of a title...}
Wir werden jetzt eine Verallgemeinerung der Taylorentwicklung in einer Variable herleiten. \\
...
\begin{align*}
\varphi:\R &\to\R^n\\
t & \mapsto \left( 1-t\right) x_0+x_1
\end{align*}
Dann ist $g:=f\circ\varphi:\R\to\R$ ($\left( g(0)=f=\left( x_0\right), g(1)=f\left( x_1\right)\right)$)eine $C^m-$Funktion und (nach Taylorentwicklung von Funktionen mit einer Variabel) gibt es $\xi\in (0,1)$, so dass
...
\[g'(t)=df\left( \varphi (t)\right) \cdot\varphi' (t)\] mit \[\varphi '(t) = {x_1} - {x_0} = \left( {{x_1}^1 - {x_0}^1,{x_1}^2 - {x_0}^2, \ldots ,{x_1}^n - {x_0}^n} \right)\]
Erhalten wir:
\begin{align*}
...
\begin{align*}
f\left( {{x_1}} \right) &= f\left( {{x_0}} \right) + \sum\limits_{i = 1}^n {\frac{{\partial f}}{{\partial x^i}}\left( {{x_0}} \right)\left( {x_1^i - x_0^i} \right) +  \ldots } \\
...
\frac{{\partial f}}{{\partial y}} =  & {e^{x + y}}\cos x ,\text{ }\frac{{\partial f}}{{\partial y}}(0,0) = 1\\
\end{align*}
...
\begin{align*}
\left( {\left( {x,y} \right) - \left( {0,0} \right)} \right){\nabla ^2}f\left( {0,0} \right){\left( {\left( {x,y} \right) - \left( {0,0} \right)} \right)^T} &= \left( {x,y} \right) \begin{pmatrix} 0&1 \\ 1&1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix} \\
 &= \left( {x,y} \right)\begin{pmatrix} y \\ x + y \end{pmatrix} \\
 &=\text{ }2xy+y^2
\end{align*}
\begin{align*}
f\left( {x,y} \right) &= {e^{x + y}}\cos x = 1 + \begin{pmatrix} 1 \\ 1 \end{pmatrix}  \left( {x,y} \right) + \frac{1}{2}\left( {x,y} \right)\begin{pmatrix} 0&1 \\ 1&1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix} \\
 &= 1 + \left( {x,y} \right) + \frac{1}{2}\left( {2xy + {y^2}} \right) + {r_3}\left( {f,\left( {x,y} \right)} \right)
\end{align*}
...
\begin{enumerate}
\item \textbf{Positiv definit} wenn \[{}^txAx = \sum\limits_{i,j = 1}^n {{a_{ij}}{x^i}{x^j} > 0}\hspace{10mm}\forall x\in\R^n\] (oder wenn sämtliche Eigenwerte positiv sind)
\item \textbf{Negativ definit} wenn \[{}^txAx < 0\hspace{10mm}\forall x\in\R^n\] (sämtliche Eigenwerte negativ)
...
\end{align*}
\begin{enumerate}
\item Falls $x_0\in\Omega$ ein lokales Extremum (Minimum oder Maximum) von $f$ ist, so gilt $df\left(x_0\right)=0$
...
\end{align*}
\[{H_f}\left( {{x_0}} \right) = \begin{pmatrix} 2&0&0 \\ 0&2&0 \\ 0&0&2 \end{pmatrix} \]
...
\[d{f^i}\left( {{x_0}} \right) = \sum\limits_{j = 1}^n {\frac{{\partial {f^i}}}{{\partial {x^j}}}\left( {{x_0}} \right)d{x^j} = \left( {\frac{{\partial {f^i}}}{{\partial x^1}}\left( {{x_0}} \right), \ldots ,\frac{{\partial {f^i}}}{{\partial {x^n}}}\left( {{x_0}} \right)} \right)} \]
die Darstellung
\[df\left( {{x_0}} \right) = \begin{pmatrix}
{\frac{{\partial f^1}}{{\partial x^1}}\left( {{x_0}} \right)}&{\frac{{\partial f^1}}{{\partial {x^2}}}\left( {{x_0}} \right)}& \ldots &{\frac{{\partial f^1}}{{\partial {x^n}}}\left( {{x_0}} \right)}\\
{\frac{{\partial {f^2}}}{{\partial x^1}}\left( {{x_0}} \right)}&{\frac{{\partial {f^2}}}{{\partial {x^2}}}\left( {{x_0}} \right)}& \ldots &{\frac{{\partial {f^2}}}{{\partial {x^n}}}\left( {{x_0}} \right)}\\
 \vdots & \vdots & \ddots & \vdots \\
{\frac{{\partial {f^l}}}{{\partial x^1}}\left( {{x_0}} \right)}&{\frac{{\partial {f^l}}}{{\partial x^2}}\left( {{x_0}} \right)}& \ldots &{\frac{{\partial {f^l}}}{{\partial {x^n}}}\left( {{x_0}} \right)}
\end{pmatrix}\]
...
\begin{enumerate}
\item \[f:\R^2\to\R\]
\[f\left( {x,y} \right) = \begin{pmatrix} {{x^2} - {y^2}} \\ {2xy} \end{pmatrix} \]
\[f\in C^\infty\left( \R^2:\R^2\right) \text{ mit } df\left( {x,y} \right) = \begin{pmatrix} {2x}&{ - 2y} \\ {2y}&{2x} \end{pmatrix}\]
\item Polarkoordinaten
\begin{align*}
f&:\lbrack 0,\infty)\times\lbrack 0,2\pi\rbrack\to\R^2\\
\begin{pmatrix} r \\ \theta \end{pmatrix} &\to \begin{pmatrix} x \\ y \end{pmatrix} = \begin{pmatrix} {r\cos \theta} \\ {r\sin \theta } \end{pmatrix} = \begin{pmatrix} {f'} \\ {{f^2}} \end{pmatrix} \\
df'\left( r,\theta\right)&=\left( \cos\theta, -r\sin\theta\right)\\
df^2\left( r,\theta\right)&=\left( \sin\theta, r\cos\theta\right)\\
df\left( {r,\theta } \right) &= \begin{pmatrix} {\cos \theta }&{ - r\sin \theta } \\ {\sin \theta }&{r\cos \theta} \end{pmatrix} \leftarrow {\text{Jacobi Matrix}}\\
...
\end{multicols}
\begin{align*}
f:&\lbrack 0,\infty)\to\lbrack 0,2\pi\rbrack\times\R\to\R^2\\
\begin{pmatrix} r \\ \theta \\ z \end{pmatrix} &\to \begin{pmatrix} x \\ y \\ z \end{pmatrix} = \begin{pmatrix} {r\cos \theta} \\ {r\sin \theta} \\ z \end{pmatrix} = \begin{pmatrix} {f'} \\ {{f^2}} \\ {{f^3}} \end{pmatrix} \\ df(r,\theta ,z) &=\left[ {\begin{array}{*{20}{c}}
...
f\begin{pmatrix} r \\ \theta \\ \psi \end{pmatrix} &= \begin{pmatrix} {r\cos \theta \cos \psi }\\ {r\sin \theta \cos \psi } \\ {r\sin \psi } \end{pmatrix} = r \begin{pmatrix} {\cos \theta \cos \psi} \\ {\sin \theta \cos \psi} \\ {\sin \psi } \end{pmatrix} \\ 
df\left( {r,\theta ,\psi } \right) &= \begin{pmatrix} {\cos \theta \cos \psi }&{ - r\sin \theta \cos \psi }&{ - r\cos \theta \sin \psi } \\ {\sin \theta \cos \psi }&{r\cos \theta \cos \psi }&{ - r\sin \theta \sin \psi} \\ {\sin \psi }&0&{r\cos \psi } \end{pmatrix} \\
...
\begin{align*}
f:\R^2 & \to\R^2\\
\left( x,y\right) & \mapsto \begin{pmatrix} {{x^2} - {y^2}} \\ {2xy} \end{pmatrix}
\hspace{20mm}df = \begin{pmatrix} {2x}&{-2y} \\ {2y}&{2x} \end{pmatrix}
\end{align*}
\begin{align*}
g:\R^3 & \to\R^2\\
\left( x,y,z\right) & \mapsto \begin{pmatrix} {{x^2} +{y^2}+{z^2}} \\ {xyz} \end{pmatrix} \hspace{15mm}dg = \begin{pmatrix} {2x}&{2y}&{2z} \\ {yz}&{xz}&{xy} \end{pmatrix}
\end{align*}
\begin{align*}
\left( f\circ g\right):\R^3 & \to\R^2\\
\left(x,y,z\right)&\mapsto \begin{pmatrix} {{{\left( {{x^2} + {y^2} + {z^2}} \right)}^2} - {{\left( {xyz} \right)}^2}} \\ {2\left( {{x^2} + {y^2} + {z^2}} \right)\left( {xyz} \right)} \end{pmatrix}
\end{align*}
\begin{align*}
d\left( {f \circ g} \right)\left( {x,y,z} \right)&= df\left( {g\left( {x,y,z} \right)} \right) \cdot dg\left( {x,y,z} \right)\\
&= \begin{pmatrix} {2\left( {{x^2} + {y^2} + {z^2}} \right)}&{ - 2xyz} \\ {2xyz}&{2\left( {{x^2} + {y^2} + {z^2}} \right)} \end{pmatrix} \begin{pmatrix} {2x}&{2y}&{2z} \\ {yz}&{xz}&{xy} \end{pmatrix} \\
&=\begin{pmatrix} {4x\left( {{x^2} + {y^2} + {z^2}} \right) - 2x{y^2}{z^2}}& * & * \\
 * & * & * \\  * & * & * \end{pmatrix}
\end{align*}
