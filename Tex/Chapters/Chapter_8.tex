...
Die Differenz $\underbrace {f(x) - P(x)}_{{d_x}_{_0}f\left( {x - {x_0}} \right)}\mathop  \to \limits_{x \to {x_0}} 0$\todo{can't understand what comes after the formula, page 126.1 middle} $P(x)$ ist eine Ebene. Die ist die Tangentialebene zu $f$ an der Stelle $x_0$ und spielt die Rolle der Tangente für Funktionen in einer Variable. \missingfigure{page 126.1 bottom}
...
\[u(t) = \int\limits_a^{b(t)} {h(s,t)ds} {\text{, }}b(t) \in\subset '\left( \R \right){\text{, }}a \in\R\]\todo{Is it $C'$ or $\subset '$?? page 146 middle; limenet: it is $C^1$}
...
\item Für jedes $f\in\subset'\left(\Omega\right)$\todo{Is it $C'$ or $\subset'$?? page 152.1 top; limenet: $C^1$} ist das Differential $df$ eine $1-Form$ \[df = \frac{{\partial f}}{{\partial {x^1}}}d{x^1} + \frac{{\partial f}}{{\partial {x^2}}}d{x^2} +  \ldots  + \frac{{\partial f}}{{\partial {x^n}}}d{x^n}\]
...
Die Richtung des Gradients ist somit \[ \nabla h\left( P \right) = \arctan{\frac{\frac{1}{5}}{\frac{\sqrt{2}}{4}+ \frac{1}{5}}}=19.86 \]\todo{add arg at the beginning of the equation using special command, as well as tilde on top of equal sign, Also add deg symbol on top of 19.86}
...
\item $\gamma (t)=\left( a\cos t,b\sin t\right)$ ist einee Parameterdarstellung einer Ellipse
\begin{figure}[H]
\begin{minipage}[b]{0.45\linewidth}
...
Eigenschaften des Wegintegrals \todo{limenet: make this bold}
...
\end{enumerate}
Die Region $\Omega=\R\backslash\{ 0\}$ ist nicht einfach zu $\Omega' = \left\{ \left( x,y\right) \mid x>0\right\}$ ist es aber.\todo{This sentence doesn't make any sense at all}
\end{definition}
...
\end{align*}
Dann ist $g:=f\circ\varphi:\R\to\R$ eine \todo[inline]{Don't know where this actually fits: $\left( g(0)=f=\left( x_0\right), g(1)=f\left( x_1\right)\right)$}$C^m-$Funktion und (nach Taylorentwicklung \todo{can't read, page 189 bottom} von Funktionen mit einer Vvariablen) gibt es $\xi\in (0,1)$, so dass
