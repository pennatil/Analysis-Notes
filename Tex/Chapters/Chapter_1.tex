...
Wir haben bewiesen, dass $A=B \to A\subset B \text{ und } B\subset A$. Zunächst nehmen wir $A\subset B$ und $B\subset A$ an. Damit möchten wir $A=B$ zeigen. \\

Sei $x\in A$, mittels $A\subset B$, haben wir $x\in B$, und somit $x\in A \to x\in B$. (\textasteriskcentered)\\

Sei $x\in B$, mittels $B\subset A$, haben wir $x\in A$, und somit $x\in B \to x\in A$. (\textasteriskcentered\textasteriskcentered)\\
...
\begin{itemize}
    ...
    \item $\cap$ verhält sich wie Multiplikation
...
Die Inverse Funktion $g:Y\to X$, einer bijektiven Funktion $f:X\to Y$ ist die Funktion, die jedem Element $y$ der Zielmenge seien eindeutig bestimmtes Urbildelement zuweist. Bei bijektiven Funktionen hat die Urbildmenge jedes Elements $y$ genau ein Element. \\
 ...
 $g(y)\defeq x$, eindeutig definiertes $ x\in\Romanbar{X}$, mit $f(x)=y$. Dann ist definitionsgemäss $(g\circ f)(x)=x$, d.h. $g\circ f:id_{\Romanbar{X}}$. Die eindeutig definierte Abbildung $g$ wird (auch) mit $f^{-1}$ bezeichnet und Umkehrfunktion von $f$ genannt.\\
