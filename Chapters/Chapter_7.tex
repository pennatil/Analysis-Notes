...
\end{definition} \todo{Where is the end of the definition??}
...
Das charakteristische Polynom der Gleichung (H) ist gegeben durch $$p(t):=t^n+a_{n-1}t^{n-1}+\dots+a_0$$
\end{definition} 
...
\begin{enumerate}
\item \todo{Where is the end of this list??}Liegt eine Linearkombination der Störfunktion vor, so hat man auch als Ansatz eine entsprechende Linearkombination zu wählen. Dies ist Superprinzip
\subsubsection*{Beispiel}
...
\subsection*{Superpositionsprinzip}\todo{Does this have to be in the enumerated list or outside??}
...
\draw[->](-0.5,0)--(3.5*pi,0);
\draw[->](0,-3.5)--(0,3.5);
\draw[white](-0.5,-0.5)--(-0.5,-7.5);%needed in order to push up the picture
...
\end{enumerate}
Eine partihuläre Lösung einer DGL $n-$ter Ordnung $$y'{(n)}+a_nx^{n-1}+\dots+a_0=b(x)$$ kann man aus der allgemeine Lösung \todo{do y and x have anything on top?? page 101 top}
...
Links und rechts stehen nur \todo{can't understand, page 110 middle} Grösse. Wenn wir aber auf der Rechten Seite nicht nur positive konstante $e^c>0$ zulassen, sondern irgendwelche Konstanten $K\in\mathbb{R}$ so erhalten wir $$y(x)=Ke^{x^2}$$
