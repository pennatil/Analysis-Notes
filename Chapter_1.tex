...
Seien $A,B$ Aussagen\\
\indent $A$ und $B$ wird mit $A\land B$ bezeichnet\\
\indent $A$ oder $B$ wird mit $A\lor B$ bezeichnet\\
\subsubsection*{Folgerung (eine wahre implikation)}
\indent Aus $A$ folgt $B$ wird mit $\Rightarrow$ bezeichnet\\
\indent Wenn $A$, dann $B$ wird mit \todo{WHAT??}\\
\indent Die negation der Aussage $A$ wird mit $\lnot A$ bezeichnet\\
\indent $A$ ist equivalent zu $B$ wird mit $A\Leftrightarrow B$ bezeichnet\\
\indent $\left(A\Rightarrow B\right)\land \left( B\Rightarrow A\right)$ $A$ wahr genau dann, wenn $B$ wahr ist.
...
\subsection*{Indirekten Beweis}\todo{Might require an enumerated list}
Man fügt $\lnot B$ als Annahme hinzu und kommt nach einer Kette von erlaubten schlüssen zu einer falschen Aussage.\\

Hieraus schliesst man, dass das zusatzannahme $\lnot B$ nicht wahr ist. 

\subsubsection*{Beispiel 1.1}
$A=\text{''jede natürliche Zahl } n \text{ hat einen Nachfolger } n+1\text{''}$\\
$B=\text{''es gibt keine grösste Naturliche Zahl''}$\\

Wir beweisen dass aus $A$ folgt $B$. Nehmen wir an, dass $A$ wahr und $B$ falsch ist. \\

$\lnot B=$ es gibt eine grösste Natürliche zahl$N_0''$ d.h $N_0>l$ für jedes $l \in \mathbb{N}$.\\
...
\begin{enumerate}
	\item \subsection*{Prinzip des Indirekten Beweises}\todo{FIX!}\\
	Zum Beweis der Aussage $A\Rightarrow B$ genügt es die Aussage $\lnot B\Rightarrow\lnot A$ zu zeigen, oder, die Annahme $A\land\lnot B$ zum Wiederspruch zu führen.
	\item \subsection*{Prinzip der Vollständigen Induktion}\todo{FIX!}\\
...
\begin{enumerate}
	\item Für alle $n\geq 1$ gilt: $$1+3+5+\dots (2n-1)=n^2\text{\indent\indent}A(n)$$
	Beweis mittels Vollständige Induktion
	\begin{enumerate}[i)]
	\item $A(i)$ lautet $1=1^2$ und gilt.
	\item Sei $n\geq 1$. Annahme: so gilt $A(n)$. Die Linke Seite der Identität $A(n+1)$ ist \[1+3+\dots (2n-1)+(2n+1)=n^{2}+(2n+1)=(n+1)^2\] womit $A(n+1)$ bewiesen ist.\todo{ERRORE!!!!!!!!!}
\end{enumerate}
...
\subsubsection*{Bemerkung}
Letztes mal haben wir gesagt jedes element von $A$ ist auch Element von $B$ ($\forall x,x\in A\Rightarrow x\in B$) wird mit $A\subset B$ ($A$ ist in $B$ enthalten, $A$ ist Teilmenge von $B$) bezeichnet. Falls $A\subset B$ und eine Element $b\in B$ gibt mit $b\not\in A$ sagen wir $A$ ist eine ''eigentliche Teilmenge'' von $B$. Manchmal schreiben wir $A\subset B$\todo{Aggiungere un notequal sotto al simbolo in} in diesem Fall. \\

Es gibt viele Bücher, mit der Folgenden Notation: jedes element von $A$ ist ein element von $B$ wird mit $\subseteq$ bezeichnet. Und wenn $A\subseteq$ und $A\not =B$ dann benutzen sie $A\subset B$ statt \todo{guardare ultimo todo}. $A=B$ falls $x\in A\Leftrightarrow x\in B$. 
...
Sei $x\in A$, mittels $A\subset B$, haben wir $x\in B$ somit $x\in A \Rightarrow x\in B$. (*)\\

Sei $x\in B$, mittels $B\subset A$, haben wir $x\in A$ somit $x\in B \Rightarrow x\in A$. (**)\\

(*) und (**) $\Rightarrow A=B$ per Definition. 
...
\draw	(-1,3) node[anchor=east] {$\Romanbar{X}$}
		(4,3) node[anchor=east] {$\Romanbar{X}$}
		(3.8,0.2) node[anchor=east] {$C$}
		;
...
\item $f:\mathbb{R}\rightarrow\mathbb{R}$
\end{enumerate}
\todo{Finish and find better way to display this list, page 18}
\begin{framed}
\centerline{\textbf{Definition: Kardinalität}}
\noindent Wir sagen zwei Mengen $X$ und $Y$ sind gleichmächtig falls eine bijektive Abbildung $f:X\rightarrow Y$ gibt.
\end{framed}
